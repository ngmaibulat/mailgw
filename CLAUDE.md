...
- **`mailgw-go/`** — a **Go** rewrite of the Haraka gateway (`emersion/go-smtp`), running **side by side** with `mailgw/` on port 2525 while parity is proven. Its own Go module, deliberately **not** a pnpm workspace member (same precedent as `logservice/` and `tests/`). Adds a declarative routing/policy rule engine, per-recipient routing and a durable outbound queue. Being moved onto **central management** (`mailgw-go/TODO.md` M3–M6): the console half shipped in `webui-fastify`; the gateway half (local wizard, SQLite config cache, config pull) is next. See its section below.
...
- **Gaps to know about:** `attachment.*`, `msg.has_attachment` and `msg.mime_part_count` are in the registry but not populated until the MIME walk lands (M8) — `check` and startup **warn** when a rule uses one. `auth.*` likewise, pending inbound AUTH. A recipient refused by a *data-stage* rule is dropped with a WARN because the SMTP reply is already spent; it should get a DSN (M7).
...
- **Central management is landing (M3–M6).** The console side is **done** in `webui-fastify` (see its section above) — a gateway registers with a self-generated Ed25519 key, an operator approves its fingerprint, and `GET /agent/config` serves an immutable versioned bundle. The gateway side is **not built yet**: M4 adds `internal/{store,central,adminui}` (pure-Go SQLite via `modernc.org/sqlite` — the image is `distroless/static` with `CGO_ENABLED=0`, so cgo drivers are out) plus `-data` / `-admin` **flags** (bootstrap is the wizard, not env vars); M5 refactors `load(dir)` in `cmd/mailgw-go/main.go` behind a `Source` interface and adds the pull loop. **`-config <dir>` file mode stays unchanged** — that is what keeps `check`, `explain`, `testdata/config`, CI and the Bun SMTP suite working. The pull reuses `watchReload`'s exact semantics: all-or-nothing, keep the running config on failure, and only allowlist + rules hot-swap (a bundle touching the relay table, listeners, TLS or the spool sets `restart_required` rather than half-applying).
