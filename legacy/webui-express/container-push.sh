...
# Bump the webui package version (under legacy/webui-express/), then build + push from
# the repo root context using the relocated Dockerfile.
( cd legacy/webui-express && pnpm version patch --no-git-tag-version )

VER=$(cd legacy/webui-express && pnpm pkg get version)

docker buildx build \
    --network=host --push \
    -f legacy/webui-express/Dockerfile \
