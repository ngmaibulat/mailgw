import { Op } from "./adapter.js";
import { BlockMD5 } from "../db/esmmodels/blockmd5.mjs";
import { HashLookup } from "../db/esmmodels/hashlookup.mjs";

export function prepareRes(data) {
...
export function createFilter(searchLogic, items) {
    if (searchLogic == "OR") {
        return {
            [Op.or]: items,
        };
...
export function createQuery(params, searchLogic, offset = 0, limit = 100) {
...
export function getData(model, request = null) {
    let limit = 100;
    let offset = 0;
    let searchParams = [];
    let searchLogic = "AND";
...
export async function checkMD5(items) {
    let md5 = [];
    ...
    let promise = BlockMD5.findAll(op);
    return promise;
}

export async function hashListLookup(list) {
    let action = "allow";
    let promises = [];
    let answers = [];

    list.forEach((txn) => {
        let tmp = hashLookup(txn).then((answer) => answers.push(answer));
        promises.push(tmp);
    });
...
export async function hashLookup(txn) {
    let md5 = txn.md5;
    let action = "allow";
    ...
    let res = await BlockMD5.findAll(op);
